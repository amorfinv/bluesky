...
class SpeedBased(ConflictResolution): 
    ...
    def SpeedBased(self, conf, ownship, intruder, idx, idx_pairs):
        ...
        for i, idx_pair in enumerate(idx_pairs):
            ...
            if (-self.front_tolerance < qdr_intruder < self.front_tolerance):
                ...
                if (abs((np.degrees(self.angle(v_ownship, v_intruder)))) > (180-self.front_tolerance)):
                    ...
                    if self.in_headon_conflict[idx] != True:
                        ...
                        stack.stack(f'LNAV {ownship.id[idx]} ON') 
                        stack.stack(f'VNAV {ownship.id[idx]} ON') 
            ...
            if (intruder.intent[idx_intruder] is not None) and (ownship.intent[idx] is not None):
        ...
        elif not can_ascend or not should_ascend:
            # Means we are most likely stuck between an aircraft. Mark this aircraft
    ...
    #@core.timed_function(name = 'check_traffic', dt=1)
    def check_traffic(self):
        """This function checks whether they are stuck behind a slow moving aircraft
        ...
        stuck_indices = np.where(self.stuck == True)
        
        for idx in stuck_indices[0]:
